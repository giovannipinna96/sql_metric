...
gold = "SELECT T2.MailStreet, T2.mailzip  FROM frpm AS T1 INNER JOIN schools AS T2 ON T1.CDSCode = T2.CDSCode ORDER BY T1.`FRPM Count (K-12)` DESC LIMIT 1"
...
gold = "SELECT T2.MailStreet, T2.mailzip  FROM frpm AS T1 INNER JOIN schools AS T2 ON T1.CDSCode = T2.CDSCode ORDER BY T1.`FRPM Count (K-12)` DESC LIMIT 1"
gen = 'SELECT T2.MailStreet FROM frpm AS T1 INNER JOIN schools AS T2 ON T1.CDSCode = T2.CDSCode ORDER BY T1.`FRPM Count (K-12)` DESC LIMIT 4'
db = 'california_schools'

# gold = "SELECT T2.MailStreet, T2.mailzip  FROM frpm AS T1 INNER JOIN schools AS T2 ON T1.CDSCode = T2.CDSCode ORDER BY T1.`FRPM Count (K-12)` DESC LIMIT 3"
# gen = 'SELECT mailstreet, mailstrabr, mailcity, mailzip FROM schools WHERE cdscode IN (SELECT cdscode FROM frpm ORDER BY "frpm count (k-12)" DESC LIMIT 1);'
# db = 'california_schools'

gold_table, gen_table, res_table, res_ves, sql_cos_sim, final_score = process_inputs(gold, gen, db)
# gold_table, gen_table, res_table, res_ves, sql_cos_sim, final_score = process_inputs(gold, 'SELECT mailzip FROM schools WHERE cdscode IN (SELECT cdscode FROM frpm ORDER BY "frpm count (k-12)" DESC LIMIT 1);', 'california_schools')
res_list.append({
    'idx': idx,
    'internal':internal,
    'sql_gold': gold,
